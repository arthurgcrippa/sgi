...
class Transformation_control(QDialog):
    ...
    def transformation_setup(self, type: int, degree: float, point: t_coordinate, vector: t_coordinate, rotate_around_axis: int, rotate_by_axis: int, object: Form, id: int):
        if object.tridimentional():
            return Transformation3D(type, degree, point, vector, rotate_around_axis, rotate_by_axis, object, id)
        else:
            return Transformation2D(type, degree, point, object, id)

    def add_translaction(self, type: int, point: t_coordinate, object: Form):
        transformation = self.transformation_setup(type, None, point, None, None, None, object, len(self.transList))
        self.transList.append(transformation)
        self.transformation_gui.transformList.addItem("Translada Objeto: " + str(object.id))

    def add_rotation(self, type: int, degree: float, point: t_coordinate, vector: t_coordinate, rotate_around_axis: int, rotate_by_axis: int, object: Form):
        transformation = self.transformation_setup(type, degree, point, vector, rotate_around_axis, rotate_by_axis, object, len(self.transList))
        self.transList.append(transformation)
